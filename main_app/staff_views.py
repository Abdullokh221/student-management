...
def staff_home(request):
    ...
    total_students = Student.objects.filter(course__in=staff.course.all()).count()
    ...
    course_names = ", ".join([course.name for course in staff.course.all()])
    context = {
        'page_title': f'Staff Panel - {staff.admin.last_name} (Courses: {course_names})',
...
def staff_feedback(request):
    staff = get_object_or_404(Staff, admin_id=request.user.id)
    form = FeedbackStaffForm(request.POST or None, staff=staff)
    context = {
        'form': form,
